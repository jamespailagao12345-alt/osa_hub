...
<div class="container">
  <div class="row justify-content-center">
    <div class="col-md-8">
      <div class="card mt-4">
        <div class="card-header" style="background-color: midnightblue; color: white;">Edit Assistant</div>
        <div class="card-body">
          @if ($errors->any())
          ...
          @endif
          <form method="POST" action="{{ route('admin.student-leaders.update', $assistant->id) }}">
            @csrf
            @method('PUT')
            <div class="mb-3">
              <label for="user_id" class="form-label">Student ID</label>
              <input type="text" class="form-control" id="user_id" name="user_id" value="{{ old('user_id', $assistant->user_id) }}" required>
            </div>
            <div class="mb-3">
              <label for="first_name" class="form-label">First Name</label>
              <input type="text" class="form-control" id="first_name" name="first_name" value="{{ old('first_name', $assistant->first_name) }}" required>
            </div>
            <div class="mb-3">
              <label for="middle_name" class="form-label">Middle Initial</label>
              <input type="text" class="form-control" id="middle_name" name="middle_name" value="{{ old('middle_name', $assistant->middle_name) }}" maxlength="1" pattern="[A-Za-z]" title="Enter only one alphabet character">
            </div>
            <div class="mb-3">
              <label for="last_name" class="form-label">Last Name</label>
              <input type="text" class="form-control" id="last_name" name="last_name" value="{{ old('last_name', $assistant->last_name) }}" required>
            </div>
            <div class="mb-3">
              <label for="contact_number" class="form-label">Contact Number</label>
              <input type="text" class="form-control" id="contact_number" name="contact_number" value="{{ old('contact_number', $assistant->contact_number) }}">
            </div>
            <div class="mb-3">
              <label for="email" class="form-label">Email</label>
              <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $assistant->email) }}" required>
            </div>
            <div class="mb-3">
              ...
              <select name="organization_id" id="organization_id" class="form-select">
                ...
                @isset($organizations)
                  @foreach($organizations as $org)
                    <option value="{{ $org->id }}" {{ old('organization_id', $assistant->organization_id) == $org->id ? 'selected' : '' }}>
                      {{ $org->name }}
                    </option>
                  @endforeach
                @endisset
            ...
            <div class="mb-3">
              ...
              <div class="border p-3 rounded" style="max-height: 200px; overflow-y: auto;">
                @php
                  // Get currently selected organization IDs for checkboxes (exclude primary org to avoid duplication)
                  $selectedOrgIds = collect();
                  if ($assistant->otherOrganizations) {
                    $selectedOrgIds = $assistant->otherOrganizations->pluck('id');
                  }
                  // If using old() data, use that, otherwise use the selected orgs
                  $oldOrgIds = old('organization_ids', $selectedOrgIds->toArray());
                @endphp
                @isset($organizations)
                  @foreach($organizations as $org)
                    <div class="form-check">
                      <input class="form-check-input org-checkbox" type="checkbox" name="organization_ids[]" id="org{{ $org->id }}" value="{{ $org->id }}"
                        {{ in_array($org->id, $oldOrgIds) ? 'checked' : '' }}>
                      <label class="form-check-label" for="org{{ $org->id }}">{{ $org->name }}</label>
                    </div>
                  @endforeach
                @endisset
              </div>
              <small class="text-muted d-block mt-2" id="org-count-message">Assistant must belong to at least 1 organization (maximum 5 total including primary). Selected: <span id="selected-count">0</span></small>
            </div>
            <div class="mb-3">
              <label for="password" class="form-label">New Password (optional)</label>
              <div class="position-relative">
                <input type="password" class="form-control" id="password" name="password" minlength="8" style="padding-right: 2.5rem;">
            ...
            <div class="d-flex gap-2">
              ...
              <a href="{{ route('admin.student-leaders.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
          </form>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  orgCheckboxes.forEach(cb => {
    cb.addEventListener('change', function() {
      if (Array.from(orgCheckboxes).filter(c => c.checked).length > 4 && this.checked) {
  ...
  primaryOrgSelect.addEventListener('change', updateCount);
  updateCount();
